...
<div class="row">
  <div class="col-12">
      <div class="card-box">
          <a href="{{route('gst-billing.create')}}" class="btn btn-sm btn-blue waves-effect waves-light float-right">
          ...
          <table class="table table-hover m-0 table-centered dt-responsive nowrap w-100 table-bordered"
              ...
              <tbody>
                @if ($gst_bills->isNotEmpty())
                @foreach ($gst_bills as $index => $gstBill)
                <tr>
                    <td><b>{{$index+1}}</b></td>
                    <td>
                        #{{$gstBill->invoice_no}}
                    ...
                    <td>
                        ...
                        <li><b>Name :</b><span> {{$gstBill->party->full_name}}</span></li>
                        <li><b>Phone :</b><span> {{$gstBill->party->phone_no}}</span></li>
                        </ul>
                    ...
                    <td>
                        <ul class="list-unstyled">
                            <li><b>Total Amount :</b><span> ₹{{number_format($gstBill->total_amount,2)}}</span></li>
                            <li><b>TAX :</b><span> ₹{{number_format($gstBill->tax_amount,2)}}</span></li>
                            <li><b>Net Amount :</b><span>₹{{number_format($gstBill->net_amount,2)}}</span></li>
                        </ul>
                    </td>

                    <td>
                        {{\Carbon\Carbon::parse($gstBill->invoice_date)->format('d M, Y')}}
                    </td>
                    <td>
                        <div class="btn-group dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item" href="{{route('gst-billing.edit',$gstBill->id)}}"><i
                                    class="mdi mdi-pencil mr-2 text-muted font-18 vertical-middle"></i>Edit</a>

                                <a class="dropdown-item" href="javascript:void(0);" onclick="deleteGstBill({{$gstBill->id}})"><i
                                        class="mdi mdi-delete mr-2 text-muted font-18 vertical-middle"></i>Delete</a>
                                <a class="dropdown-item" href="{{route('gst-billing.show',$gstBill->id)}}"><i
      ...
      <div class="col-12">
        {{-- pagination --}}
      {{$gst_bills->links('pagination::bootstrap-5')}}
...
$("#searchForm").submit(function(e) {
    ...
    var url = '{{route("gst-billing.index")}}?';
  ...
  $("#keyword").change(function() {
    $("#searchForm").submit();
      ...
      // delete data
   function deleteGstBill(id) {
    url = '{{route("gst-billing.destroy","ID")}}';
